...
Сидит, значит, Татьяна Владимировна Ким в главном офисе своей компании **"Дикая Ягодка"**, и в её кабинет забегает **Ильдар Мангушев** (главный поставщик айфонов по всей России). Ильдар взволнован и просит, чтобы в его **личном кабинете поставщика** была нормальная информация о:  
- продажах,  
- возвратах,  
- карточках его товаров.  

Долго умолять не пришлось. Лицо Татьяны приобрело вид, подобный этому: 😈. Отправила она поставщика домой и сказала, чтобы он не беспокоился.  

А сама взяла телефон, набрала своих разработчиков и выдвинула им следующее **ТЗ (техническое задание)**:  

#### Необходимо разработать **сервис подписок для продавцов**.  
### Суть подписки  
Подписка должна предоставлять **дополнительные возможности (лимиты)**, которые будут **недоступны продавцам без подписки**.  

Примеры лимитов:  

### ✅ **Limit 1 — Две карточки для одного товара**  
Дает возможность создать **две карточки для одного и того же товара** и сравнивать, **по какой карточке товар продается лучше**. Это позволит продавцам проводить A/B-тестирование своих товаров.  

---

### ✅ **Limit 2 — Теги на товары**  
Позволяет **ставить теги на свои товары**, чтобы можно было **помечать несколько товаров на одной фотографии**. Например, на одном фото могут быть и чехол для айфона, и зарядка, и защитное стекло — с возможностью привязки к разным карточкам.  

---

### ✅ **Limit 3 — Видео вместо фото**  
Вместо стандартных фотографий товара, продавец сможет загружать **видео с обзором на товар**. Это значительно повысит конверсию и доверие покупателей.  

---

### ✅ **Limit 4 — Деление товаров на категории**  
Продавец сможет **разделить товары по категориям**. Например:  
- **Мама** покупает подгузники своему ребенку и переходит в каталог товаров продавца, чтобы посмотреть, есть ли подгузники другого размера.  
- Если у продавца подключен **Limit 4**, то мама **не увидит товары другой категории**, например, игрушки 18+ (реальный кейс, сами были в шоке, когда узнали, угорали всей командой, но такой лимит создали).  

---

## Итог  
Татьяна Владимировна поручила разработать этот сервис **как можно быстрее**, чтобы продавцы, такие как Ильдар Мангушев, могли **получить доступ к расширенным возможностям** и повысить свои продажи,ну и конечно повысить доход компании "дикая ягодка" 

**Сроки сжаты. Работаем.** 🚀  
...
### 4. Запуск миграций и контейнеров ( ОБЯЗАТЕЛЬНО ВЫПОЛНИТЬ ВСЕ КАК ПОКАЗАНО))
...
### Доп задание для самых одареных
   Если думаешь, что самый умный то задание такое:
   * Роняешь докер и все сервисы;
   * Поднимаешь докер, залетаешь на локальный хост rabbitMQ;
   * Создаешь очерешь и добавляешь новый тип обработчика, по желанию естественно;
   * Дописываешь код сервисов, подсказку оставлю;
   * Стартуешь и проверяешь;
   * Протестируй со всеми типами обменника;

### Вопросы для защиты

1. Виды обменников в RabbitMQ
2. Какой протокол TCP/API используется в RabbitMQ
3. Рассказать чем отличаются обменники между собой
4. Какой обменник мы не можем указать при настройки rabbitMQ?
