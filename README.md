...
### Предыстория
Наша компания "Ягодка" предоставляет подписки продавцам. Подписка включает в себя основной доступ и дополнительные лимиты, позволяющие продавцам анализировать продажи и сравнивать товары.

В сервисе подписок есть API `ActiveSubscription`, который создаёт запись о подписке в базе данных и отправляет сообщение в RabbitMQ о её активации. Далее сервис лимитов распределяет лимиты на подписку.
...
RabbitMQ и PHP
RabbitMQ является кроссплатформенным и может использоваться на различных операционных системах. Интеграция с различными инструментами разработки осуществляется через библиотеки.
Для использования RabbitMQ с PHP рекомендуется использовать библиотеку с имплементацией модели протокола AMQP 0-9-1 – php-amqplib.
RabbitMQ поддерживает передачу различных типов сообщений, таких как текстовые сообщения, бинарные данные, изображения, аудио и видео. При этом формат сообщений может быть любым, но наиболее распространенными форматами являются JSON, XML, и plain text.
...
4. После успешного запуска приложение должно быть активным.
...
1. Откройте терминал (Ctrl + J в VSCode).
...
### 5. Запуск сервисов
#### 5.1 Запуск миграций
1. В новом терминале снова перейдите в `migrations`.
...
1. Установите расширение для работы с PostgreSQL.
